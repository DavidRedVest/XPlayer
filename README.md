...
- **录像封装格式切换** ——TS / MKV / MP4 三选一（右键菜单里的"录制格式"子菜单），默认 MP4；TS/MKV 不强依赖"干净收尾才能播放"，进程被杀/异常断电时已经写下去的部分仍然可读，MP4 因为索引（`moov`）只在收尾时一次性写完，异常中断这一段基本报废。
...
- **每格独立选择声音 + 全局音量** ——单击某一格让它出声音，实时声音永远跟随当前单击选中的格子；顶部滑块（带百分比数字）控制全局音量，持久化保存。
- **截图** ——右键菜单"截图"，直播、回放格子都能用，按视频源原始分辨率（不受格子在屏幕上显示大小限制）存成 PNG，保存到独立的 `screenshots` 目录，同名文件自动加序号、不会覆盖。
...
另外还有两个小型命令行工具，可以在不构建 GUI 的情况下直接跑 `xcodec`，方便把解码引擎的问题和 GUI 的问题分开排查：`tools/xcodec_probe`（打开一路流、打印帧统计、退出）和 `tools/xrecorder_probe`（用指定的封装格式录一段指定时长，验证 `XRecorder` 本身）。
...
tools/
  ...
  xrecorder_probe/        XRecorder 录像功能的命令行冒烟测试工具
```

## 更新日志

版本历史、每个版本改了什么、开发过程中遇到的问题和解决方案，见 [CHANGELOG.md](CHANGELOG.md)。
