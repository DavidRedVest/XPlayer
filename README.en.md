...
Every version tag (e.g. `v0.1.0`) is automatically built for Windows/Linux/macOS via GitHub Actions and published to this repo's [Releases](../../releases) page — just unpack and run, no dependency setup needed. The macOS build is ad-hoc (temporarily) signed; on first launch you'll need to allow it once under System Settings → Privacy & Security.
...
| Windows | Not yet built/verified on real hardware — no known platform-specific blockers in source, CI pipeline for Windows builds is set up |
| Linux | Not yet built/verified on real hardware — no known platform-specific blockers in source, CI pipeline for Linux builds is set up |
